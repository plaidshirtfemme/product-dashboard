...
def get_dash_issues() -> list[dict]:
    ...
    issues += [
        _di("DASH-1", "Изучить целевую вакансию и определить концепцию демо",
    ...
    issues += [
        ...
        _di("DASH-120", "Research: свод правил дизайн-системы по канону (Figma + документация)",
            ...
            decision_note=(
                ...
                "в CLAUDE.md). Канон: токены — единый источник (DTCG 2025.10); 3 уровня primitive→"
        ...
        _di("DASH-121", "Research: как дизайнеры и фронтендеры синхронят работу и дизайн-системы",
            ...
            description=(
                ...
                "(Figma variables → Tokens Studio → JSON → код, DTCG); handoff-процесс; "
            ...
            decision_note=(
                ...
                "НЕ МОГЛИ разойтись (не «Figma главный» и не «код главный», а токены). Формат: DTCG "
                "2025.10 (первая стабильная версия, окт. 2025) — один JSON-формат $value/$type, который Figma, Tokens "
        ...
        _di("DASH-63", "Hi-fi макеты в Figma на основе wireframes",
            ...
            description=(
                ...
                "Демонстрирует Figma proficiency для вакансии Company 1.\n\n"
        ...
        _di("DASH-80", "UX/UI аудит дашборда: пройти по всем вкладкам глазами рекрутера",
            ...
            description=(
                ...
                "ТЮНИНГ РАСКЛАДКИ ТАБЛИЦ (перенесено из DASH-114, решение Guzel 25.07 — базовая "
                "механика работает, дальше это вопрос вкуса/полировки, смотреть глазами рекрутера):\n"
                "1. Reorder вставляет колонку ВСЕГДА перед целевой. Нет индикации «до/после» — "
                "по позиции курсора в левой/правой половине заголовка (как в Jira/AG Grid).\n"
                "2. Ресайз показывает направляющую линию, а не живую ширину колонки (осознанный "
                "размен ради скорости на 118 строках). Пересмотреть, если покажется непривычным.\n"
                "3. Wrap включён во всех ячейках → строки разной высоты, таблицу труднее сканировать. "
                "Возможен line-clamp (2-3 строки) вместо полного переноса.\n"
                "4. Мин. ширина = ширина шапки. У иконочных колонок (🔒, 📊) шапка крошечная — "
                "можно сузить почти в ноль. Нужен отдельный разумный минимум.\n"
                "5. Перестановка только мышью — нет клавиатурной альтернативы (в dnd-kit она есть). "
                "Вопрос a11y, не блокер для витрины.\n"
                "6. Touch/тачпад-жесты не проверялись (pointer-события должны работать, но не тестировали).\n"
                "7. Кнопка «Сбросить раскладку» держит строку 26px, даже когда скрыта — микро-джиттер layout.\n"
                "8. Раскатка reorderable_table на ОСТАЛЬНЫЕ таблицы дашборда (staged, по одной с ревью). "
                "Какие блоки вообще должны стать таблицами — сначала DASH-146 (ревизия 29 grid-блоков "
                "по критерию «однородные записи + осмысленные колонки» vs «просто раскладка»)."
            )),
        ...
        _di("DASH-95", "Перенарезка эпиков по ценностям: business/enabler, распределение задач",
            ...
            decision_note=(
                ...
                "«Кейс → оффер Company 1» — главная цель; «удобный дашборд для выдуманной команды» — инструментальная "
        ...
        _di("DASH-151", "Цвет сущности «эпик» захардкожен: purple мимо системы токенов",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 3, 3, "Guzel K.",
            created="2026-09-08T18:00:00.000+0000",
            labels=["design-system", "tech-debt"], priority="Medium",
            description=(
                "Найдено 08.09 при заведении слоя состояний (см. DASH-147 и коммит "
                "слоя color.state).\n\n"
                "ЗАМЕР: purple встречается 6 раз, все в backlog.py — бейдж «epic» "
                "(color_scheme=\"purple\"), ключ эпика rx.color(\"purple\", 11) и три "
                "ховера ссылок на эпики. Это не случайный цвет: purple последовательно "
                "означает «сущность эпик». Но живёт он захардкоженным в одном файле, "
                "мимо design_tokens.json.\n\n"
                "ПОЧЕМУ ЭТО ТА ЖЕ БОЛЕЗНЬ, ЧТО И СОСТОЯНИЯ: цвет несёт смысл, значит "
                "принадлежит семантическому слою. Рядом уже есть EPIC_TYPE_COLORS "
                "(business/enabler/component → teal/amber/gray) — но это ТИП эпика, "
                "а не сама сущность. Две разные вещи, и вторая нигде не описана.\n\n"
                "РЕШИТЬ ПЕРЕД ПРАВКОЙ: purple вообще не входит в тему (accent teal, "
                "gray slate). Либо это осознанное исключение — тогда завести его "
                "в токенах именно как исключение, с обоснованием; либо это случайное "
                "решение — тогда заменить на цвет из темы. Второе меняет вид Backlog, "
                "поэтому смотреть глазами до и после.\n\n"
                "Не блокер. Но ровно этот класс находок вакансия называет словами "
                "«найти дубли, противоречия и случайные решения»."
            )),

        _di("DASH-152", "Состояния focus и disabled: нужны ли продукту, и если да — из чего брать значения",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 3, 2, "Guzel K.",
            created="2026-09-08T18:00:00.000+0000",
            labels=["design-system", "a11y", "ux"], priority="Medium",
            description=(
                "Продолжение DASH-147. 08.09 заведён слой color.state из пяти токенов, "
                "значения сняты с фактического кода. Focus и disabled НЕ заведены "
                "сознательно: цветов под них в коде нет, а придумать значение ради "
                "полноты категории — хуже, чем назвать пробел вслух.\n\n"
                "ЭТО ЗАДАЧА НА РЕШЕНИЕ, А НЕ НА ПРАВКУ. Сначала ответить, нужны ли "
                "эти состояния продукту вообще:\n\n"
                "FOCUS. Дашборд read-only, но по нему ходят с клавиатуры: 8 мест "
                "с cursor=pointer, аккордеон навигации, переставляемые колонки "
                "(cursor=grab, 5 мест). Если focus-состояния нет, клавиатурный "
                "пользователь не видит, где он находится. Связано с DASH-150 "
                "(доступность по охвату) — там рамка «берём массовое»; вопрос, "
                "попадает ли focus-ring в эту рамку или это нишевое.\n\n"
                "DISABLED. Основание уже есть: в navigation.py у непостроенных "
                "вкладок ховер отключён (_hover=... if is_built else {}), и в README "
                "записано «WIP-вкладки показываются серым» — то есть визуальное "
                "различие в продукте ЕСТЬ, но выражено не токеном, а условием "
                "в компоненте. Здесь значение можно снять с кода, как со всеми "
                "остальными.\n\n"
                "ПОРЯДОК: сначала disabled (основание в коде есть), потом focus "
                "(нужно решение по рамке доступности). Если решено, что focus "
                "не нужен — записать это решением, а не оставлять молча.\n\n"
                "УТОЧНЕНО 09.09 замером. DISABLED: значение снимается — "
                "navigation.py, непостроенная вкладка = color gray.7 против "
                "gray.10, cursor default, _hover={}, on_click=None, тултип. "
                "FOCUS: снять неоткуда (ноль вхождений _focus/:focus/"
                "focus_visible вне .web), НО состояние не факультативное — "
                "у Material оно одно из пяти обязательных, а Radix публикует "
                "шаг 7 как «UI element border and focus rings». Значит "
                "заводить решением по канону, с пометкой что принято, "
                "а не снято. Прежняя формулировка «цветов под них в коде нет» "
                "была неверна для disabled — поправлена в tokens.py и на "
                "вкладке DS."
            )),

        _di("DASH-153", "Аудит значений: соблюдается ли граница Radix ↔ наши токены",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 5, 5, "Claude Code",
            created="2026-09-09T19:00:00.000+0000",
            labels=["design-system", "tech-debt", "audit"], priority="High",
            description=(
                "ПРАВИЛО, КОТОРОЕ ПРОВЕРЯЕМ: «Radix Themes — примитивный слой; "
                "мы владеем semantic, role и state; чего нет в наших токенах — "
                "берётся из Radix по умолчанию». Сейчас правило нигде "
                "не записано как правило и, судя по замеру, не соблюдается.\n\n"
                "ПОЧЕМУ ЗАВЕДЕНА (Guzel, 09.09): значения вне Radix Themes "
                "не согласовывались. Их вводил ассистент по ходу сборки "
                "интерфейса и придумывал новые на месте. То есть в продукте "
                "есть слой решений без автора.\n\n"
                "ЗАМЕР 09.09 (dash_app, без jira_mock_raw и sprint_calendar):\n"
                "— rx.color(...) прямых вызовов: 1035; через COLOR_ROLE/ROLE/"
                "STATE: 4. Семантический слой заведён (DASH-141 P2), но "
                "консюмеры на него не мигрированы — миграция была намечена "
                "как P3 и не сделана.\n"
                "— строк вида \"NNpx\": 575; обращений SPACING[...]: 562.\n"
                "— захардкоженных line_height/letter_spacing: 72, из них "
                "девять разных интерлиньяжей (1.6 ×22, 1.5 ×21, 1.7 ×6, 1.4 ×3, "
                "1.35 ×2, 1.3 ×2, 1.55, 1.45, 1) и три letter-spacing "
                "(0.04em ×6, 0.06em ×4, 0.05em ×3).\n"
                "— hex-цветов: 0 (закрыто в DASH-141 P1).\n\n"
                "ЗНАЧЕНИЯ ВНЕ ОБЕИХ ШКАЛ. Наша шкала отступов: 2,4,8,16,24,32,48. "
                "Radix: 4,8,12,16,24,32,40,48,64. В коде часто встречаются "
                "6px (68), 10px (33), 3px (13) — их нет НИ в одной из двух. "
                "Плюс 12px (32) и 20px (11) — есть у Radix, нет у нас.\n\n"
                "ОГОВОРКА К ЗАМЕРУ: 575 — это все строки с px, включая ширины "
                "(1100px, 160px, 60px), толщины рамок и размеры иконок. Не все "
                "они про отступы. Первый шаг задачи — разделить их по назначению, "
                "и только потом считать нарушения. Мерить надо целое, а не "
                "сразу фильтровать.\n\n"
                "ПРИЁМКА: (1) правило записано первым классом — в CLAUDE.md "
                "и на вкладке DS; (2) есть таблица «значение → где встречается → "
                "чьё решение → куда его отнести (Radix / наш токен / удалить)»; "
                "(3) по каждому значению вне обеих шкал принято решение Guzel, "
                "а не ассистента; (4) 6px, 10px, 3px либо получили основание, "
                "либо заменены на ближайший шаг."
            )),

        _di("DASH-154", "TYPE_SCALE: перевести три места на rx.text(size=N) вместо font_size",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 2, 2, "Claude Code",
            created="2026-09-09T19:10:00.000+0000",
            labels=["design-system", "tech-debt"], priority="Medium",
            description=(
                "СДЕЛАНО 09.09: из TYPE_SCALE удалены два мёртвых токена — "
                "size-body (14px) и size-page-title (22px). Ни один компонент "
                "их не потреблял, жили только на странице документации. "
                "22px к тому же лежал вне шкалы Radix, между шагами 5 (20px) "
                "и 6 (24px). Токенов стало 41 вместо 43.\n\n"
                "ОСТАЛОСЬ. Три оставшихся размера совпадают с шагами Radix "
                "(label 12px = шаг 1, heading 18px = шаг 4, value 24px = шаг 6), "
                "но потребляются через style={\"font_size\": ...}, минуя "
                "rx.text(size=\"N\"). Из-за этого текст не получает парных "
                "line-height и letter-spacing, которые Radix отдаёт вместе "
                "с шагом («designed to be used in combination»).\n\n"
                "МЕСТА: components/section_header.py:25 (heading), "
                "components/stat_card.py:53 (label), "
                "components/stat_card.py:70 (value). Плюс демо-строки "
                "в pages/ds.py — их менять последними.\n\n"
                "ЭТО ВИЗУАЛЬНАЯ ПРАВКА, НЕ МЕХАНИЧЕСКАЯ. Переход на size=\"N\" "
                "добавит интерлиньяж, которого сейчас нет, и высота карточек "
                "и заголовков поедет. Приёмка — глазами, до/после, по одному "
                "месту за раз. Под дедлайн не делать.\n\n"
                "ВОПРОС К РЕШЕНИЮ: если после перехода TYPE_SCALE перестанет "
                "нести что-либо сверх шкалы Radix — удалять его целиком "
                "или оставить как именованный слой ролей."
            )),

        _di("DASH-155", "Достижимость с клавиатуры: кликабельные div не попадают в порядок обхода",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 5, 5, "Claude Code",
            created="2026-09-09T19:30:00.000+0000",
            labels=["a11y", "ux", "tech-debt"], priority="High",
            description=(
                "ЗАМЕР НА ЖИВОЙ СТРАНИЦЕ 09.09, вкладка Kanban, локальный "
                "reflex run. Перепроверено на ЧИСТОМ старте сервера и в чистой "
                "вкладке браузера: 0 фокусируемых в DOM, role — 0 во всём "
                "документе, tabindex — 1 элемент, и тот со значением -1:\n"
                "— фокусируемых элементов на странице: 0. Селектор "
                "a[href], button, input, select, textarea, [tabindex не -1], "
                "[contenteditable] не даёт НИ ОДНОГО видимого совпадения. "
                "Нажатие Tab не попадает никуда.\n"
                "— пункты сайдбара: тег DIV, без tabindex, без role.\n\n"
                "ЗАМЕР ПО КОДУ (dash_app, без jira_mock_raw и sprint_calendar):\n"
                "— 20 обработчиков on_click, 7 мест с cursor=\"pointer\";\n"
                "— tab_index: 0 вхождений; role=: 0 вхождений;\n"
                "— при этом 24 rx.select, 5 rx.button и 1 rx.link работают "
                "нормально: они рендерятся настоящими контролами, браузер сам "
                "ставит их в порядок обхода, Radix Themes рисует кольцо фокуса.\n\n"
                "ПРИЧИНА. Кликабельные элементы собраны как rx.flex(...) "
                "с on_click, то есть на выходе <div>. Div не входит в порядок "
                "обхода, не реагирует на Enter и пробел, и скринридер "
                "не называет его кнопкой. Человек, идущий по интерфейсу "
                "с клавиатуры, не может открыть ни одну вкладку.\n\n"
                "ПОЧЕМУ ЭТО НЕ КОСМЕТИКА. Принцип Microsoft из Playbook: "
                "«Creating products for people with permanent disabilities "
                "creates results that benefit everyone», и числа Persona "
                "Spectrum — 26 тысяч против 20 миллионов, если считать "
                "временные и ситуативные ограничения. Клавиатура — это "
                "не только скринридер: сломанная мышь, тачпад, быстрая "
                "навигация, работа одной рукой.\n\n"
                "ДВА СПОСОБА ЧИНИТЬ.\n"
                "(A) КАНОНИЧНЫЙ, рекомендуемый: заменить кликабельные rx.flex "
                "на rx.button(variant=\"ghost\"). Тогда достижимость, "
                "активация с клавиатуры и кольцо фокуса приходят из Radix "
                "бесплатно, а свой токен focus нужен только если захочется "
                "другое кольцо. Меньше кода и меньше своего.\n"
                "(B) ЗАПЛАТКА: оставить div и добавить tab_index=0, "
                "role=\"button\", aria_current для активного пункта, "
                "обработку Enter и пробела, видимое кольцо фокуса. Больше "
                "ручной работы и легче забыть половину.\n\n"
                "СВЯЗИ. DASH-152 — токен focus: до этой задачи он бесполезен, "
                "красить нечего, пока фокус не встаёт. DASH-150 — замеры "
                "доступности: там про контраст и размеры, это другая ось, "
                "задачи не дублируются.\n\n"
                "ПОРЯДОК: сначала навигация (главная интерактивная поверхность, "
                "3 места с cursor=pointer в navigation.py), потом остальные "
                "кликабельные. Тикет — завершаемая единица: если после "
                "навигации остаток окажется низкоценным, закрыть и завести "
                "новый по факту.\n\n"
                "ПРИЁМКА: (1) Tab проходит по всем интерактивным элементам "
                "в осмысленном порядке; (2) Enter и пробел активируют то же, "
                "что клик; (3) кольцо фокуса видно; (4) повторный замер "
                "селектором focusable даёт не 0, а ожидаемое число; "
                "(5) проверено вручную с клавиатуры, а не только замером."
            )),
        ...
        _di("DASH-107", "Публикация кейса + отклик Company 1 (буфер 18-20.07: Company 3)",
            "To Do", "Task", "PM", _DASH_EPICS["E12"], 4, 2, "Guzel K.",
            created="2026-07-11T10:00:00.000+0000",
            labels=["release"], priority="Highest",
            description="⏰ Жёсткий дедлайн вс 19.07 (сдвинут с 17.07). Отклики Company 1 + Company 3 ≤ 22.07."),
        ...
        _di("DASH-114", "Backlog: drag-n-drop порядок колонок + ручная ширина колонок",
            "Done", "Story", "DEV", _DASH_EPICS["E9"], 5, 8, "Claude Code",
            created="2026-07-11T18:00:00.000+0000",
            started="2026-07-24T19:00:00.000+0000",
            resolved="2026-07-25T13:00:00.000+0000",
            ...
            decision_note=(
                "РЕШЕНИЕ 24-25.07. Сделано переиспользуемым ядром на ВСЕ таблицы (решение Guzel), а не "
                "точечно в Backlog: components/reorderable_table.py (Column-реестр + рендер через "
                "foreach(порядок)+match(id)) + states/table_layout_state.py (общий стейт, rx.LocalStorage "
                "по table_id, дефолты в Python-реестре, в стейте только оверрайды → новая колонка в коде "
                "не ломает сохранённую раскладку). Применено к Backlog Issues (16 колонок) и Epics (12), "
                "у каждой свой table_id. Раскатка на остальные таблицы — staged, отдельными заходами "
                "(часть «таблиц» — flex-вёрстка, они приходят через DS-консолидацию DASH-70/92).\n"
                "ГЛАВНЫЙ ВЫВОД (2 итерации фидбека Guzel): HTML5 drag-n-drop, который РАБОТАЕТ в "
                "div-вёрстке (таймлайн motif_about), внутри <table>/<th> НЕ работает вообще. Ресёрч "
                "индустрии подтвердил: dnd-kit (официальные Column-DnD примеры TanStack Table) и "
                "Atlassian Pragmatic DnD (Jira/Trello/Confluence) не используют HTML5 drag API — только "
                "pointer-события. Переведены на pointer оба взаимодействия через rx.call_script с Promise "
                "(фронтенд await'ит промис и отдаёт результат в callback).\n"
                "Три бага из фидбека и их причины: (1) грип ресайза и draggable-заголовок дрались за "
                "события → убран draggable совсем, грип и ярлык — соседние узлы, грип поверх по z-index; "
                "(2) тормоза на 118 строках → ресайз перерисовывал всю таблицу на каждый mousemove "
                "(усугублено переносом текста) → заменён на направляющую линию, ширина применяется один "
                "раз на отпускание, reflow во время drag = 0 (замер: 25 событий = 2 мс), плюс мин. ширина "
                "= ширина шапки и wrap контента; (3) захват «залипал» → причина requestAnimationFrame "
                "(не тикает в фоновых вкладках), убран; снятие захвата продублировано на pointercancel и "
                "window.blur. Отдельно найден баг: коммит без реального перетаскивания писал фантомный "
                "оверрайд и затирал сохранённую ширину → добавлен флаг moved.\n"
                "Побочно: aria-describedby варнинги на React.Fragment признаны предсуществующими и "
                "глобальными (есть на голом шелле без таблиц) → вынесены в DASH-145, из дифа 114 убраны.\n"
                "ЗАКРЫТА 25.07 по приёмке Guzel («стало лучше и то и то»). Дальнейшая полировка "
                "(индикация до/после при вставке, live-ширина vs направляющая, line-clamp вместо wrap, "
                "минимум для иконочных колонок, клавиатурная перестановка, touch, раскатка на остальные "
                "таблицы) перенесена в DASH-80 — это уже вкусовой тюнинг, смотреть глазами рекрутера."
            )),
        ...
        _di("DASH-117", "Провенанс данных: честный real/demo на переключателе продукта",
            "Done", "Story", "PM", _DASH_EPICS["E15"], 4, 3, "Guzel K.",
            created="2026-07-11T20:30:00.000+0000",
            started="2026-07-20T13:00:00.000+0000",
            resolved="2026-07-20T14:00:00.000+0000",
            ...
            description=(
                ...
                "(реальные проекты, исполнители Guzel+Claude, задачи оформлены вручную), а метились как "
                "«Демо · модель Jira-интеграции» — занижали."
            ),
            decision_note=(
                "РЕШЕНИЕ 20.07 (Guzel, переоценка scope): не аудит 59 секционных бейджей, а прямое устранение "
                "самой лжи. Переключатель продукта (navigation.py _project_dropdown) уже честно показывает "
                "«Real · этот дашборд как продуктовый проект» / «Real · соло-пайплайн» / «Demo · вымышленная "
                "команда» — текстом. Значит корневая ложь решена на уровне единственного видимого источника "
                "правды о происхождении данных. Все 59 секционных data_source_badge (real/mock) УДАЛЕНЫ из "
                "рендера (18 файлов pages/): action=... параметры и 4 нестандартных вызова, вместе с мёртвыми "
                "импортами. Заодно закрыт хвост решения A от 11.07 (временная метка mock на календаре спринта "
                "kanban.py — убрана вместе с комментарием). section_header.action — Optional, layout не пострадал."
                "\n\nПочему не мешает работе с DS: убранные бейджи не рендерятся → не блокируют DASH-80/81/92/93 "
                "(наоборот, чище заголовки секций перед hi-fi). Реактивный бейдж для двухрежимных backlog/about "
                "стал не нужен — нечему быть нереактивным.\n\n"
                "Гранулярная 3-состояний таксономия (extracted/logged/demo по каждой секции) — вынесена в "
                "техдолг DASH-137, не блокирует ничего."

            )),
        _di("DASH-137", "Техдолг: гранулярный провенанс extracted/logged/demo по секциям",
            "To Do", "Task", "PM", _DASH_EPICS["E15"], 4, 2, "Claude Code",
            created="2026-07-20T14:00:00.000+0000",
            labels=["tech-debt", "data"], priority="Low",
            description=(
                "Ответвление от DASH-117 (сужена 20.07). Единый честный real/demo бейдж на переключателе "
                "продукта уже решает корневую проблему (DASH-117 Done). Эта задача — про более тонкую, "
                "не срочную нюансировку: у _SOURCE_MAP до сих пор только real/mock (2 состояния), а честнее "
                "было бы различать 3: extracted (данные из реального источника — vault_snapshot, "
                "real_project_extract) / logged (реальная работа, задачи ведём вручную — KP + DASH) / "
                "demo (вымышленный проект — только Motif). Раньше это подразумевало ~60 секционных бейджей "
        ...
        _di("DASH-147", "Баг: 🔒 «заблокирована» горит у закрытых задач (18 из 30) + сверить направление связей",
            "Done", "Bug", "DEV", _DASH_EPICS["E15"], 5, 5, "Claude Code",
            created="2026-07-25T14:30:00.000+0000",
            started="2026-09-08T15:00:00.000+0000",
            resolved="2026-09-08T17:30:00.000+0000",
            labels=["ux", "data"], priority="Medium",
            decision_note=(
                "Закрыт 08.09. Первая половина: правило «заблокирована» вынесено "
                "в производное поле Issue.blocked_active (сама не Done + есть блокер не Done), "
                "заполняется в load_issues(), когда известны статусы всех задач. Причина выноса — "
                "та же неверная проверка `if i.blocked_by` стояла в ШЕСТИ местах: Backlog, Kanban "
                "(иконка и счётчик), Dev, metrics (blocked по сквадам, overall_blocked_rate, "
                "critical_blocked). Патчить каждое значило бы завести шесть версий одного правила. "
                "Замер: 30 замков → 11, закрытых среди них 0. "
                "SimpleIssueRow тоже получил поле, иначе вкладка Dev падала на AttributeError — "
                "поймано сквозным прогоном, а не деплоем.\n\n"
                "Вторая половина: аудит 45 связей по двум независимым сигналам — хронология "
                "(ключ Done, блокер нет) и комментарий рядом со строкой. Помечено 19, все "
                "в перенарезках 12 и 18.07; ранняя цепочка 44→45→47→54→55 заведена верно. "
                "Развёрнуты, направление взято из комментариев. DASH-105 развёрнут вместе "
                "с 103/104 сверх списка: та же цепочка к сборке 106,наполовину развёрнутая семья хуже. "
                "Найдены и слиты ДВА дубля ключей в links_map — DASH-64 и DASH-65: второй ключ "
                "молча затирал первый, часть связей не существовала. decision_note нигде "
                "не переписывались, как оговорено в задаче.\n\n"
                "После правки: хронологически невозможных 0 (было 9), дублей ключей 0, "
                "приложение импортируется целиком, замков в DASH 8 — все на незакрытых."
            ),
            description=(
                "Найдено 25.07 при заведении связей для DASH-146.\n\n"
                "СИМПТОМ (замер): в Backlog колонка 🔒 показывает красный замок у 30 задач, из них "
                "18 в статусе Done. «Закрыта, но заблокирована» — противоречие, которое видит рекрутер.\n"
                "ПРИЧИНА: adapter.blocked_by — статический список зависимостей из issuelinks; "
                "backlog.py рисует иконку по `blocked != ''`, не проверяя, закрыт ли блокер. "
                "Правильная логика: замок = есть блокер, который ЕЩЁ НЕ Done (и сама задача не Done). "
                "Так это и работает в реальной Jira.\n\n"
                "СВЕРИТЬ ЗАОДНО — направление связей. `_block(key)` кладёт inwardIssue → адаптер читает "
                "как blocked_by, то есть «A»: [_block(«B»)] означает «A заблокирована B». Для части пар "
                "это верно (DASH-45 blocked_by DASH-44 — 44 действительно раньше), но для других "
                "противоречит комментариям и CLAUDE.md: там сказано «124 (аудит) блокирует 107 "
                "(публикацию)» и «70 → blocks → 92 → blocks → 63», а закодировано наоборот "
                "(124 blocked_by 107, 70 blocked_by 92). Пройти links_map и привести направление к "
                "намерению; НЕ переписывать при этом исторические decision_note.\n\n"
                "Не блокер витрины, но дешёвый фикс с заметным эффектом на достоверность доски."
            )),
        _di("DASH-146", "DS: что вообще должно быть таблицей — ревизия 29 grid-блоков",
            "To Do", "Task", "DESIGN", _DASH_EPICS["E17"], 5, 3, "Guzel K.",
            created="2026-07-25T14:00:00.000+0000",
            labels=["design-system", "architecture", "ux"], priority="Medium",
            description=(
                "Продолжение линии DASH-70/92 (обе закрыты 24.07) — вопрос всплыл при раскатке "
                "reorderable_table (DASH-114) и НЕ должен решаться по инерции «привести всё к таблицам».\n\n"
                "ВОПРОС: все ли 29 grid-блоков (table_header/table_row на rx.grid) вообще должны "
                "становиться настоящими таблицами? Часть из них, вероятно, честная сетка-раскладка, а не "
                "табличные данные. Тащить их в <table> ради единообразия = переусложнение; канон DS прямо "
                "предупреждает: правильный atomic design даёт МЕНЬШЕ компонентов, а не больше.\n\n"
                "КРИТЕРИЙ (зафиксирован в CLAUDE.md, раздел «Таблицы: базовый компонент + свойства»): "
                "настоящая таблица = однородные записи + колонки со своим смыслом, которые осмысленно "
                "сортировать / переставлять / скрывать. Всё остальное — раскладка, ей достаточно grid.\n\n"
                "ЛАНДШАФТ на 25.07 (замер, не память): reorderable_table — 2 (Backlog Issues/Epics) · "
                "прямые rx.table.root — 4 (motif_about, kanban, info, dash_discover) · статический "
                "data_table — 3 вызова (columns: list[str] + готовые ячейки, БЕЗ id колонок → это миграция, "
                "а не «подключение») · grid-блоки — 29 использований.\n\n"
                "СДЕЛАТЬ: пройти 29 блоков по критерию → разметить «таблица / раскладка»; для попавших в "
                "«таблица» — план миграции на reorderable_table (нужны id колонок и per-column рендереры); "
                "для «раскладки» — явно зафиксировать, что остаются grid, чтобы к вопросу не возвращаться. "
                "Результат питает staged-раскатку (пункт 8 в DASH-80) и hi-fi (DASH-63)."
            )),
        _di("DASH-145", "Техдолг: aria-describedby на React.Fragment — аудит Fragment-триггеров Radix",
            "To Do", "Task", "DEV", _DASH_EPICS["E15"], 5, 2, "Claude Code",
            created="2026-07-24T20:00:00.000+0000",
            labels=["tech-debt", "ux"], priority="Low",
            description=(
                "Симптом: консоль в dev залита «Invalid prop `aria-describedby` supplied to "
                "React.Fragment. React.Fragment can only have `key` and `children` props».\n"
                "Причина: Radix-компонент с asChild (триггер tooltip / dropdown_menu / dialog / "
                "popover) получает ребёнка, который рендерится Fragment'ом — rx.cond / rx.match / "
                "rx.foreach / rx.fragment (часто через функцию-хелпер, возвращающую cond). Radix "
                "пытается прокинуть aria-describedby в Fragment → варнинг.\n"
                "Доказано (ответвление от DASH-114, 24.07): варнинги есть уже на голом шелле "
                "(сайдбар/switcher, БЕЗ таблицы Backlog), а ре-рендер таблиц Backlog даёт 0 новых "
                "(React дедупит prop-варнинги) → источник ГЛОБАЛЬНЫЙ, в шелле приложения, "
                "предшествует 114. В DOM 0 элементов с aria-describedby → влияния на прод/UX/a11y "
                "нет, это чисто dev-шум.\n"
                "Сделать: найти все Fragment-триггеры (grep не ловит — идут через хелперы; искать "
                "по рендер-стеку / поочерёдным отключением tooltip'ов в nav/kanban/router-хелперах), "
                "обернуть проблемные триггеры в rx.box (inline-flex) или отдать не-Fragment. Включить "
                "колонку tracking в backlog.py (rx.tooltip(rx.cond(...))) — латентный вклад таблицы, "
                "откачен из 114 сюда. Аккуратно: обёртка не должна ломать layout/hover."
            )),
        ...
        _di("DASH-156", "ADR: выбор фреймворка дашборда — Streamlit → Reflex",
            "Done", "Task", "ARCH", _DASH_EPICS["E17"], 1, 2, "Guzel K.",
            created="2026-06-23T10:00:00.000+0000", started="2026-06-28T10:00:00.000+0000",
            resolved="2026-07-02T22:19:00.000+0000",
            labels=["architecture", "process"], priority="High",
            description=(
                "Ретроспективный ADR (заведён 12.09.2026): зафиксировать, почему дашборд строится на "
                "Reflex, а не на Streamlit, с которого начинали. Пробел обнаружен при раскопках сессий — "
                "разговор о самом выборе в сохранённых транскриптах не нашёлся (доступный архив сессий "
                "начинается 23.05.2026 и не содержит KP-сессий, где строился Streamlit-прототип). "
                "Решение закрыть пробел честной записью, а не оставлять реестр ADR без фундамента."
            ),
            decision_note=(
                "РЕШЕНИЕ: дашборд на Reflex (Python→React), а не на Streamlit. "
                "ПОДТВЕРЖДЁННЫЕ ФАКТЫ (артефакты, не память): (1) дашборд НАЧИНАЛСЯ на Streamlit — "
                "память проекта knowledge-pipeline на 23.06.2026 дословно: «Дашборд Streamlit начат "
                "(dashboard.py), план 10 страниц по ролям команды»; (2) Reflex-пересборка датирована "
                "именем корневой папки «20260702_2219_kp_dashboard_v2» → 02.07.2026. Значит переход "
                "Streamlit→Reflex произошёл в окне 23.06–02.07.2026. "
                "ОГОВОРКА ПО ЧЕСТНОСТИ: точная дата и формулировка момента выбора в транскриптах НЕ "
                "сохранились; даты выше — границы по двум реальным артефактам, сам рационал ниже "
                "РЕКОНСТРУИРОВАН из свойств инструментов и ограничений проекта, а не из записи того "
                "разговора. "
                "РАЦИОНАЛ (реконструкция): Streamlit — rerun-модель (весь скрипт перевыполняется на "
                "каждое действие), слабый контроль вёрстки/роутинга/состояния — не тянет "
                "многовкладочный портфолио-дашборд с кастомной дизайн-системой и 3 режимами проекта. "
                "Reflex — компонентная модель с явным State и настоящим роутингом, кастомный UI прямо "
                "на токенах (критично для кейса Product Designer — дизайн-система как источник правды), "
                "деплой одной командой на Reflex Cloud (2 процесса, без Docker — см. раздел деплоя). "
                "Плюс единый язык: пайплайн KP уже на Python → фронт остаётся в Python, front-end "
                "компетенция для дизайн-роли без ухода в JS. "
                "СЛЕД В ПРОЕКТЕ: подтверждается тем, что вся витрина — нативные Reflex-компоненты на "
                "токенах (design_tokens.json DTCG ↔ tokens.py), а Figma→code-генерация сознательно НЕ "
                "используется именно потому, что все такие тулы выводят JS, а не Reflex (см. DASH-119)."
            )),
        ...
        _di("DASH-134", "Подготовка к живому Figma-тесту (Company 2): auto-layout, variants, tokens",
            ...
            description=(
                "ГЕЙТ вакансии Company 2: живой 10-минутный тест на Zoom — собрать сложное модальное окно "
        ...
        _di("DASH-135", "Отклик Company 2: портфолио-ссылка через Telegram-бот",
        ...
        _di("DASH-136", "Чек-лист аккуратности перед откликом (гейт качества портфолио и дашборда)",
            ...
            created="2026-07-19T16:00:00.000+0000",
            labels=["ux", "process", "release"], priority="Highest",
            description=(
                "ЗАЧЕМ: у Company 2 портфолио-ссылка = сама заявка, смотрят сеньора и оценивают "
                "АККУРАТНОСТЬ; первое впечатление не отозвать. Поэтому доделывать в спешке ПОСЛЕ отклика "
                "нельзя (решение Guzel 19.07 — она поправила план «откликаться на 80%»). Одновременно "
                "чек-лист задаёт КОНЕЧНУЮ планку, чтобы полировка не стала бесконечной (её паттерн-риск).\n"
                "ПЛАНКА «презентабельно» — право сказать «готово»:\n"
                "1) Первый экран + те 3-5 вкладок, которые реально откроют: консистентные отступы/"
                "типографика/токены, НОЛЬ placeholder / lorem / шаблонных блоков.\n"
                "2) Глубокие вкладки: либо аккуратно, либо честный empty_state — полусобранного не "
                "показываем.\n"
                "3) Нет битых ссылок; свежий деплой; проверено на мобильном.\n"
                "4) Framer: Home + Mirum-кейс полные, без заглушек; mobile-tile — либо готов, либо не "
                "показываем вовсе.\n"
