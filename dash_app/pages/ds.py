        ...
        rx.box(
            ...
            *[_state_row(name, scale, step) for name, (scale, step) in COLOR_STATE.items()],
            rx.text(
                ...
                "не назначены заново. Focus и disabled отсутствуют по разным "
                "причинам (уточнено замером 09.09). Focus: снять неоткуда — ноль "
                "вхождений в коде, но состояние не факультативное (у Material "
                "оно одно из пяти), и значение документировано каноном: шаг 7 "
                "Radix это «UI element border and focus rings». Заводить "
                "решением, с пометкой что принято, а не снято. Отдельно и "
                "важнее: у select, button и link фокус даёт Radix и он работает, "
                "а свои кликабельные без tab_index и role с клавиатуры "
                "недостижимы вообще — это дефект доступности, кольцом он "
                "не чинится. Disabled: состояние есть и значение снимается — "
                "непостроенная вкладка навигации это gray.7 против gray.10, "
                "снятый ховер и снятый клик; ждём решения DASH-152.",
