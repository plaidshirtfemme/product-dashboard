...
def _epic_type_chip(epic_type, unlocks) -> rx.Component:
    """Тип эпика (DASH-93). rx.badge(color_scheme=DynamicVar) не работает →
    rx.box + rx.cond по конвенции проекта."""
    ...
    return rx.cond(
        ...
        rx.box(
            rx.text(label, size="1", weight="medium",
                    color=rx.cond(
                        epic_type == "business", rx.color("teal", 11),
                        rx.cond(epic_type == "enabler",
                                rx.color("amber", 11), rx.color("gray", 10)),
                    )),
            background=rx.cond(
                epic_type == "business", rx.color("teal", 3),
                rx.cond(epic_type == "enabler",
                        rx.color("amber", 3), rx.color("gray", 3)),
            ),
            padding="1px 8px",
            border_radius="var(--radius-full)",
